...
* **Till exempel?** Det kan vara ett välkomstmeddelande när du har skapat ett konto, en bekräftelse på att en beställning har skickats, en faktura, ett meddelande som bekräftar en lösenordsändring eller ett meddelande efter att en kund har bläddrat på webbplatsen, osv.
...
Transaktionsmeddelanden kan skickas via e-post, SMS eller [push-meddelanden](../../channels/using/transactional-push-notifications.md), beroende på vilka alternativ du har. Kontrollera licensavtalet.
...
**Händelsetransaktionsmeddelanden** måldata som finns i själva händelsen. Dessa meddelanden:
...
**Profiltransaktionsmeddelanden** målprofiler från kampanjmarknadsföringsdatabasen. Med den här typen av meddelanden kan du:
* Utnyttja data i Adobe Campaign-databasen.
* Anpassa meddelandet med profilinformation genom att lägga till en [anrikning](../../channels/using/configuring-transactional-event.md#enriching-the-transactional-message-content) i händelsekonfigurationen.
...
Meddelandetypen definieras när händelsen som ska omvandlas till ett transaktionsmeddelande konfigureras.        Se konfigurationsavsnitten [Händelsebaserade transaktionsmeddelanden](../../channels/using/configuring-transactional-event.md#event-based-transactional-messages) och [Profilbaserade transaktionsmeddelanden](../../channels/using/configuring-transactional-event.md#profile-based-transactional-messages).
...
| --- |--- |--- |--- |
| <img src="assets/do-not-localize/icon_notification.svg" width="40px"> | Det här steget kan utföras av en marknadsföringsanvändare som har [administrationsbehörighet](../../administration/using/users-management.md#functional-administrators). | Redigera och anpassa transaktionsmeddelandet, testa det och publicera det sedan. | Transaktionsmeddelandet är sedan klart att skickas. |
...
När du har skapat en händelse måste du integrera den som utlöser händelsen på webbplatsen.<!--In this example, you want a "Cart abandonment" event to be triggered whenever one of your clients leaves your website before purchasing the products in their cart.--> Webbplatsutvecklaren måste använda **Adobe Campaign Standard REST API** för att göra detta.
