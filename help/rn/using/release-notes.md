...
<p>OBS! Den här förändringen kommer gradvis att lanseras för alla kundmiljöer mellan 10 februari och 1 mars.
</p>
</td> 
</tr> 
</tbody> 
...
* Vi har åtgärdat ett problem som innebar att leveransanalysen misslyckades när ett push-transaktionsmeddelande som riktades till profiler skickades. Detta gjorde vi genom att introducera en ny [målmappningsprofil](../../administration/using/target-mappings-in-campaign.md) **– realtidshändelse för pushmeddelanden** (*mapRtEventAppSubRcp*). Loggarna för leverans, uteslutning och spårning av [push-transaktionsmeddelanden som riktas till en profil](../../channels/using/transactional-push-notifications.md#transactional-push-notifications-targeting-a-profile) kommer nu att lagras i tabellerna *broadLogAppSubRcp*, *excludeLogAppSubRcp* och *trackingLogAppSubRcp*.
